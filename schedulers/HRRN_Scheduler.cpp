...
class HRRN_Scheduler {
	...
	HRRN_Scheduler(MY_Queue<Process> job_queue) : num_of_process(0), running(NULL_PROCESS), using_io(NULL_PROCESS),
	...
	void dispatch(int t){
		...
		float max_rr = 0;
		Process tmp = *ready_queue.begin();
		for (const auto& proc : ready_queue) {
			float tmp_value = (float)(t - proc.last_entered_ready_queue) / proc.bursts[proc.index_of_burst];
			if(tmp_value > max_rr){
				tmp = proc;
				max_rr = tmp_value;
			}
